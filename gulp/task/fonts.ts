...
export const otfToTtf = (done: TransformCallback) => {
  ...
  if (!fs.existsSync(app.path.src.fonts)) {
    console.log(
      colors.yellow(
        `Внешних шрифтов в проекте не обнорожено. Директория ${app.path.src.fonts} не обноружена или пуста.`,
      ),
    );
    done();
    return;
  ...
  if (!fs.existsSync(app.path.tmp.fonts)) {
    fs.mkdirSync(app.path.tmp.fonts, { recursive: true });
  }

  return (
    app.gulp
      .src(app.path.src.fontsOtf, { encoding: false })
      .pipe(
        app.plugins.plumber(
...
export const ttfToWoff = (done: TransformCallback) => {
  ...
  return (
    app.gulp
      .src(app.path.src.fontsTtf, { encoding: false })
      .pipe(
        app.plugins.plumber(
...
export const fontToCss = (done: TransformCallback): void => {
  ...
  if (fs.existsSync(fontsScssPath)) {
    console.log(
      colors.yellow(
        `Шрифты уже подключены. Чтобы конвертировать их повторно удалите файл ${fontsScssPath}`,
      ),
    );
    done();
    return;
