...
export const img = () => {
  return (
    app.gulp
      ...
      .pipe(app.gulp.src(app.path.src.img, { encoding: false })) // Повторный выбор исходных изображений.
      ...
      .pipe(
        app.plugins.if(
          // Выполняется только в продакшн режиме.
          app.isProd,
          app.gulp.src(app.path.src.img, { encoding: false }), // Получаем доступ к изображениям (снова).
        ),
      )
      .pipe(
        app.plugins.if(
